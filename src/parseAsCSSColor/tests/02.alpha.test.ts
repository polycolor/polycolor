...
describe('alpha', () => {
  describe('color as number with alpha', () => {
    ...
    test('parse(0xFF0000, .5)', () => {
      expect(parse(0xFF0000, 0.5)).toBe('rgba(255,0,0,0.5)')
    })
    ...
    test('parse(0x123456, .5)', () => {
      expect(parse(0x123456, 0.5)).toBe('rgba(18,52,86,0.5)')
    })
  })
  ...
  describe('color as string with alpha', () => {
    ...
    test('parse(#123456, .5)', () => {
      expect(parse('#123456', 0.5)).toBe('rgba(18,52,86,0.5)')
    })
  })
  ...
  describe('CSS color keywords', () => {
    ...
    test('parse(red, .5)', () => {
      expect(parse('red', 0.5)).toBe('rgba(255,0,0,0.5)')
    })
